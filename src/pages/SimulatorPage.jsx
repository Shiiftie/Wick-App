...
// Price-tracking P&L overlay
// We track a running history of prices to estimate where "current price" sits
// vertically in the visible chart window
function PnlOverlay({ positions, price, decimals, chartHeight }) {
  // Track min/max price seen to estimate vertical position
  ...
  // Reserve ~30% of chart height for indicators at bottom
  const chartVisibleHeight = chartHeight * 0.62
  const priceRange = maxPrice - minPrice

  const priceToY = (p) => {
    if (priceRange === 0) return chartVisibleHeight / 2
    return chartVisibleHeight - ((p - minPrice) / priceRange) * chartVisibleHeight
  }

  const currentY = Math.max(20, Math.min(chartVisibleHeight - 20, priceToY(price)))
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 10, overflow: 'hidden' }}>
      {positions.map((pos, i) => {
        ...
        const entryY = Math.max(20, Math.min(chartVisibleHeight - 20, priceToY(pos.entry)))
        ...
        return (
          <g key={pos.id}>
            ...
            <div style={{
              ...
              left: 0, right: 0,
              height: '1px',
              background: `repeating-linear-gradient(90deg, rgba(232,200,74,0.5) 0px, rgba(232,200,74,0.5) 6px, transparent 6px, transparent 12px)`,
            }} />
            {/* Entry label */}
            <div style={{
              position: 'absolute',
            ...
            {/* Current price line — follows actual price */}
            <motion.div
            ...
            {/* P&L label — floats on current price line */}
            <motion.div
              animate={{ top: `${currentY - 14}px` }}
              transition={{ type: 'spring', stiffness: 120, damping: 20 }}
              style={{
                position: 'absolute',
                right: '70px',
                background: profit ? 'rgba(0,20,10,0.92)' : 'rgba(20,0,5,0.92)',
            ...
            {/* Connector line between entry and current price */}
            {Math.abs(currentY - entryY) > 4 && (
              <div style={{
                position: 'absolute',
                left: '50%',
                top: `${Math.min(currentY, entryY)}px`,
                width: '1px',
                height: `${Math.abs(currentY - entryY)}px`,
...
export default function SimulatorPage({ user }) {
  ...
  const handleLog = async (trade) => {
    ...
    await supabase.from('sessions').insert({
      ...
      analysis: `Demo — ${trade.asset.label} ${trade.direction.toUpperCase()} ${trade.entry.toFixed(decimals)} → ${trade.closePrice?.toFixed(decimals)}`,
    ...
    <div style={{ fontFamily: 'monospace', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 90px)', overflow: 'hidden', background: '#080808' }}>
      ...
      <div style={{ height: '40px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', flexShrink: 0, background: '#0c0c0c' }}>
        ...
        {[
          ...
          <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0 16px', borderRight: '1px solid rgba(255,255,255,0.05)', height: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0 16px', borderRight: '1px solid rgba(255,255,255,0.05)', height: '100%' }}>
      ...
      {/* ── BODY ── */}
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: `120px 1fr ${showPanel ? '210px' : '0px'}`, overflow: 'hidden', minHeight: 0, transition: 'grid-template-columns 0.25s ease' }}>

        {/* ── LEFT: Asset list ── */}
        <div style={{ borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#0a0a0a' }}>
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
            {Object.keys(ASSETS).map(cat => (
              <button key={cat} onClick={() => { setCategory(cat); setAsset(ASSETS[cat][0]) }}
                style={{ flex: 1, padding: '8px 0', background: 'transparent', border: 'none', borderBottom: category === cat ? '2px solid #e8c84a' : '2px solid transparent', color: category === cat ? '#e8c84a' : 'rgba(255,255,255,0.25)', fontSize: '8px', fontWeight: '700', letterSpacing: '0.5px', cursor: 'pointer', textTransform: 'uppercase' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {ASSETS[category].map(a => (
              <button key={a.symbol} onClick={() => setAsset(a)}
                style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', padding: '8px 10px', background: asset.symbol === a.symbol ? 'rgba(232,200,74,0.07)' : 'transparent', border: 'none', borderLeft: asset.symbol === a.symbol ? '2px solid #e8c84a' : '2px solid transparent', cursor: 'pointer', transition: 'all 0.1s', gap: '2px' }}>
        ...
        {/* ── CENTER: Chart (bleeds fully) ── */}
        <div ref={chartRef} style={{ position: 'relative', overflow: 'hidden', minHeight: 0, minWidth: 0 }}>
          {/* Extend chart slightly beyond container to hide internal TV padding */}
          <div style={{ position: 'absolute', top: 0, bottom: 0, left: '-1px', right: '-1px' }}>
          ...
          <motion.button whileTap={{ scale: 0.95 }} onClick={() => setShowPanel(!showPanel)}
            style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 20, background: 'rgba(0,0,0,0.75)', border: '1px solid rgba(232,200,74,0.25)', borderRadius: '6px', padding: '5px 11px', color: '#e8c84a', fontSize: '9px', fontWeight: '700', cursor: 'pointer', letterSpacing: '1px', backdropFilter: 'blur(8px)' }}>
        ...
        {/* ── RIGHT: Order panel ── */}
        {showPanel && (
          <div style={{ borderLeft: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#0a0a0a' }}>
            <div style={{ flex: 1, padding: '12px 11px', overflowY: 'auto' }}>
              <div style={{ fontSize: '8px', color: 'rgba(255,255,255,0.18)', letterSpacing: '2px', marginBottom: '10px' }}>ORDER — {asset.label}</div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px', marginBottom: '10px' }}>
                {['long', 'short'].map(d => (
                  <motion.button key={d} whileTap={{ scale: 0.96 }} onClick={() => setDirection(d)}
                    style={{ padding: '10px 0', borderRadius: '7px', border: `1px solid ${direction === d ? (d === 'long' ? '#00ff88' : '#ff4466') : 'rgba(255,255,255,0.07)'}`, background: direction === d ? (d === 'long' ? 'rgba(0,255,136,0.12)' : 'rgba(255,68,102,0.12)') : 'rgba(255,255,255,0.02)', color: direction === d ? (d === 'long' ? '#00ff88' : '#ff4466') : 'rgba(255,255,255,0.3)', fontSize: '11px', fontWeight: '800', cursor: 'pointer', letterSpacing: '1.5px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
              ...
              <div style={{ marginBottom: '9px' }}>
                <label style={{ fontSize: '8px', color: 'rgba(255,255,255,0.2)', letterSpacing: '1.5px', display: 'block', marginBottom: '4px' }}>SIZE ($)</label>
                <input type="number" value={size} onChange={e => setSize(e.target.value)}
                  style={{ width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '6px', color: '#e0e0e0', fontSize: '13px', fontWeight: '700', outline: 'none', boxSizing: 'border-box', fontFamily: 'monospace' }} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px', marginBottom: '10px' }}>
                <div>
                  <label style={{ fontSize: '8px', color: '#ff6680', letterSpacing: '1px', display: 'block', marginBottom: '3px' }}>SL</label>
                  <input type="number" value={sl} onChange={e => setSl(e.target.value)} placeholder="—"
                    style={{ width: '100%', padding: '6px 7px', background: 'rgba(255,68,102,0.04)', border: '1px solid rgba(255,68,102,0.12)', borderRadius: '5px', color: '#ff6680', fontSize: '11px', outline: 'none', boxSizing: 'border-box', fontFamily: 'monospace' }} />
                </div>
                <div>
                  <label style={{ fontSize: '8px', color: '#00cc6a', letterSpacing: '1px', display: 'block', marginBottom: '3px' }}>TP</label>
                  <input type="number" value={tp} onChange={e => setTp(e.target.value)} placeholder="—"
                    style={{ width: '100%', padding: '6px 7px', background: 'rgba(0,255,136,0.04)', border: '1px solid rgba(0,255,136,0.12)', borderRadius: '5px', color: '#00cc6a', fontSize: '11px', outline: 'none', boxSizing: 'border-box', fontFamily: 'monospace' }} />
                </div>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 9px', background: 'rgba(255,255,255,0.02)', borderRadius: '6px', marginBottom: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              ...
              {positions.length > 0 && (
                <>
                  <div style={{ fontSize: '8px', color: 'rgba(255,255,255,0.18)', letterSpacing: '2px', marginBottom: '7px' }}>OPEN ({positions.length})</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                    <AnimatePresence>
                      {positions.map(pos => {
                        ...
                        const color = profit ? '#00ff88' : '#ff4466'
                        return (
                          <motion.div key={pos.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }}
                            style={{ background: profit ? 'rgba(0,255,136,0.04)' : 'rgba(255,68,102,0.04)', border: `1px solid ${profit ? 'rgba(0,255,136,0.1)' : 'rgba(255,68,102,0.1)'}`, borderRadius: '7px', padding: '8px 9px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                              <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                                ...
                                <span style={{ fontSize: '11px', fontWeight: '700', color: '#b0b0b0' }}>{pos.asset.label}</span>
                              </div>
                              <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleClose(pos.id)}
                                style={{ background: 'rgba(255,68,102,0.1)', border: '1px solid rgba(255,68,102,0.2)', borderRadius: '4px', padding: '2px 7px', color: '#ff4466', fontSize: '9px', fontWeight: '700', cursor: 'pointer' }}>
                                CLOSE
                              </motion.button>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                              <div>
                                ...
                                <div style={{ fontSize: '10px', fontWeight: '700', color: '#909090' }}>{pos.entry.toFixed(decimals)}</div>
                              </div>
                              <div style={{ textAlign: 'right' }}>
                                <div style={{ fontSize: '7px', color: 'rgba(255,255,255,0.18)', marginBottom: '1px' }}>P&L</div>
              ...
              {history.length === 0 ? (
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                    <thead>
                      <tr>
                        {['ASSET', 'DIR', 'ENTRY', 'EXIT', 'P&L', 'TIME', ''].map(h => (
