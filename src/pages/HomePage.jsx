...
function Avatar({ url, username, size = 24, color }) {
  return (
    <div style={{
      width: size, height: size, borderRadius: '50%',
      overflow: 'hidden', flexShrink: 0,
      background: 'linear-gradient(135deg, #7c5cfc, #e8c84a)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      fontSize: size * 0.42, fontWeight: '800', color: '#fff',
      border: color ? `2px solid ${color}40` : 'none',
      boxSizing: 'border-box'
    }}>
      {url
        ? <img src={url} alt="av" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            <div key={msg.id} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
              <Avatar url={userAvatars[msg.user_id]} username={msg.username} size={26} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '1px' }}>
                  <span style={{ fontWeight: '700', fontSize: '12px', color: userColors[msg.user_id] || '#e8c84a' }}>{msg.username}</span>
                  <span style={{ color: 'var(--text-muted)', fontSize: '10px' }}>{formatTime(msg.created_at)}</span>
                </div>
                <p style={{ fontSize: '13px', color: '#e0e0e0', lineHeight: '1.5', wordBreak: 'break-word' }}>{msg.content}</p>
              </div>
            </div>
          ))
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {leaders.map((l, i) => (
            <div key={l.username || i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: i < 3 ? '14px' : '11px', width: '18px', textAlign: 'center', color: 'var(--text-muted)', fontWeight: '700', flexShrink: 0 }}>{icons[i]}</span>
              <Avatar url={l.avatar_url} username={l.username} size={28} />
              ...
              <span style={{ fontSize: '11px', fontWeight: '700', color: 'var(--gold)', flexShrink: 0 }}>{(l.xp || 0).toLocaleString()}</span>
            </div>
          ))}
        </div>
      )}
