...
import { Home, Trophy, Users, User, LogOut, MessageSquare, BarChart2, Newspaper, Target, BookOpen, Menu, X } from 'lucide-react'
...
export default function Navbar({ view, setView, user, onLogout }) {
  ...
  return (
    <>
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100 }}>
        ...
        <nav style={{
              ...
              {navItems.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <motion.button key={id} onClick={() => setView(id)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 14px', borderRadius: '8px', border: 'none', background: active ? 'rgba(232,200,74,0.1)' : 'transparent', color: active ? '#e8c84a' : 'rgba(255,255,255,0.35)', fontSize: '13px', fontWeight: active ? '700' : '400', cursor: 'pointer', transition: 'color 0.2s, background 0.2s', boxShadow: active ? 'inset 0 0 12px rgba(232,200,74,0.08)' : 'none' }}
                    onMouseEnter={e => { if (!active) { e.currentTarget.style.color = 'rgba(232,200,74,0.7)'; e.currentTarget.style.background = 'rgba(232,200,74,0.05)' } }}
                    onMouseLeave={e => { if (!active) { e.currentTarget.style.color = 'rgba(255,255,255,0.35)'; e.currentTarget.style.background = 'transparent' } }}
                  >
                    {active && <div style={{ position: 'absolute', inset: 0, borderRadius: '8px', background: 'radial-gradient(ellipse at 50% 100%, rgba(232,200,74,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />}
                    <Icon size={14} strokeWidth={active ? 2.5 : 1.8} style={{ filter: active ? 'drop-shadow(0 0 4px rgba(232,200,74,0.7))' : 'none' }} />
                    {label}
                    {active && (
                      <motion.div layoutId="activeTab"
                        style={{ position: 'absolute', bottom: '0px', left: '8px', right: '8px', height: '2px', borderRadius: '2px 2px 0 0', background: 'linear-gradient(90deg, transparent, #e8c84a, transparent)', boxShadow: '0 0 10px 2px rgba(232,200,74,0.6), 0 0 20px 4px rgba(232,200,74,0.3)' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
            ...
            {/* Mobile hamburger for extra pages */}
            {isMobile && (
      ...
      {/* Mobile slide-down menu for extra pages */}
      <AnimatePresence>
        {isMobile && menuOpen && (
          <motion.div
            ...
            {navItems.map(({ id, label, icon: Icon }) => {
              ...
              return (
                <motion.button key={id} whileTap={{ scale: 0.97 }}
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderRadius: '10px', border: 'none', background: active ? 'rgba(232,200,74,0.1)' : 'transparent', color: active ? '#e8c84a' : 'rgba(255,255,255,0.6)', fontSize: '14px', fontWeight: active ? '700' : '400', cursor: 'pointer', textAlign: 'left', width: '100%' }}>
                  <Icon size={16} strokeWidth={active ? 2.5 : 1.8} />
                  {label}
                  {active && <div style={{ marginLeft: 'auto', width: '6px', height: '6px', borderRadius: '50%', background: '#e8c84a', boxShadow: '0 0 6px rgba(232,200,74,0.8)' }} />}
                </motion.button>
              )
            })}
          ...
          {bottomTabItems.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <motion.button key={id} whileTap={{ scale: 0.9 }} onClick={() => { setView(id); setMenuOpen(false) }}
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '3px', flex: 1, height: '100%', background: 'transparent', border: 'none', cursor: 'pointer', color: active ? '#e8c84a' : 'rgba(255,255,255,0.3)', position: 'relative' }}>
                {active && (
                  <motion.div layoutId="bottomTab"
                    style={{ position: 'absolute', top: 0, left: '20%', right: '20%', height: '2px', borderRadius: '0 0 2px 2px', background: 'linear-gradient(90deg, transparent, #e8c84a, transparent)', boxShadow: '0 0 8px rgba(232,200,74,0.6)' }}
                    transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                  />
                )}
                <Icon size={20} strokeWidth={active ? 2.5 : 1.8} style={{ filter: active ? 'drop-shadow(0 0 4px rgba(232,200,74,0.7))' : 'none' }} />
