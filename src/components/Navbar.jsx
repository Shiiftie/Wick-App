  ...
  useEffect(() => {
    ...
    const fetchTickerData = async () => {
      ...
      try {
        ...
        const { data: recentWins } = await supabase.from('sessions').select('pnl, profiles:user_id(username)').eq('outcome', 'win').order('created_at', { ascending: false }).limit(4)
        ...
        {isMobile && menuOpen && (
          ...
          >
            ...
            <motion.button whileTap={{ scale: 0.97 }}
              ...
              <Bot size={16} />
              VEGA — AI Assistant
              <div style={{ marginLeft: 'auto', width: '7px', height: '7px', borderRadius: '50%', background: '#e8c84a', boxShadow: '0 0 6px rgba(232,200,74,0.8)' }} />
            </motion.button>
