      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            style={{ position: "fixed", bottom: 110, right: 24, zIndex: 999, width: 310, background: "rgba(6,6,6,0.97)", border: "1px solid rgba(232,200,74,0.25)", borderRadius: 16, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.8), 0 0 0 1px rgba(232,200,74,0.05)" }}
          >
            {/* Header */}
            <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(232,200,74,0.03)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button
                  ...
                  style={{ background: muted ? "rgba(255,68,102,0.15)" : "rgba(232,200,74,0.08)", border: "1px solid " + (muted ? "rgba(255,68,102,0.4)" : "rgba(232,200,74,0.2)"), borderRadius: 6, color: muted ? "#ff4466" : "rgba(232,200,74,0.6)", cursor: "pointer", padding: "4px 8px", fontSize: 10, fontFamily: "monospace" }}
                >
                  {muted ? "🔇" : "🔊"}
                </button>
              ...
              {error
                ...
                : response
                  ...
                  : <div style={{ fontSize: 12, color: "rgba(255,255,255,0.25)", fontFamily: "monospace" }}>
                      {isListening ? (displayTranscript || "Listening...") : isThinking ? "Thinking..." : "Speak or type to talk to WICK AI."}
                    </div>
              }
            </div>
            ...
            <div style={{ padding: "0 16px 10px", display: "flex", gap: 6 }}>
              <button
                ...
                style={{ flex: 1, padding: "6px", background: inputMode === "voice" ? "rgba(232,200,74,0.12)" : "rgba(255,255,255,0.03)", border: "1px solid " + (inputMode === "voice" ? "rgba(232,200,74,0.4)" : "rgba(255,255,255,0.08)"), borderRadius: 7, color: inputMode === "voice" ? gold : "rgba(255,255,255,0.3)", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "monospace", letterSpacing: 1 }}
              >🎙 VOICE</button>
              <button
                onClick={() => setInputMode("text")}
                style={{ flex: 1, padding: "6px", background: inputMode === "text" ? "rgba(232,200,74,0.12)" : "rgba(255,255,255,0.03)", border: "1px solid " + (inputMode === "text" ? "rgba(232,200,74,0.4)" : "rgba(255,255,255,0.08)"), borderRadius: 7, color: inputMode === "text" ? gold : "rgba(255,255,255,0.3)", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "monospace", letterSpacing: 1 }}
              >⌨️ TEXT</button>
            </div>
              ...
              <div style={{ padding: "0 16px 14px", display: "flex", gap: 8 }}>
                ...
                {response && (
                  <button
                    ...
                    style={{ padding: "10px 14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 9, color: "rgba(255,255,255,0.3)", fontSize: 11, cursor: "pointer", fontFamily: "monospace" }}
                  >CLR</button>
                )}
              </div>
            )}
            ...
            {inputMode === "text" && (
              <form onSubmit={handleTextSubmit} style={{ padding: "0 16px 14px", display: "flex", gap: 8 }}>
                ...
                <button
                  ...
                  style={{ padding: "9px 14px", background: "rgba(232,200,74,0.1)", border: "1px solid rgba(232,200,74,0.3)", borderRadius: 9, color: gold, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "monospace", opacity: isThinking || !textInput.trim() ? 0.4 : 1 }}
                >SEND</button>
              </form>
            )}
          </motion.div>
