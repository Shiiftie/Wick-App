...
export default function WickAIButton({ user, externalOpen, onExternalClose }) {
  ...
  useEffect(() => {
    if (!isOpen && onExternalClose) onExternalClose()
  }, [isOpen])
  ...
  useEffect(() => {
    ...
    r.onresult = (e) => {
      ...
      pauseTimerRef.current = setTimeout(() => { r.stop() }, 5000)
    }
    r.onend = () => {
      if (pauseTimerRef.current) clearTimeout(pauseTimerRef.current)
  ...
  return (
    <>
      {/* Only show floating button on desktop */}
      {!isMobile && (
        <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 1000 }}>
          <motion.div animate={{ scale: [1, 1.4, 1], opacity: [0.4, 0, 0.4] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
            style={{ position: "absolute", inset: -8, borderRadius: "50%", border: "2px solid rgba(232,200,74,0.5)", pointerEvents: "none" }} />
          <motion.div animate={{ scale: [1, 1.7, 1], opacity: [0.3, 0, 0.3] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut", delay: 0.4 }}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div key="vega-panel" initial={{ opacity: 0, y: 16, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 16, scale: 0.96 }}
            style={{ position: "fixed", bottom: isMobile ? 0 : 110, right: isMobile ? 0 : 24, left: isMobile ? 0 : "auto", zIndex: 999, width: isMobile ? "100%" : 310, background: "rgba(6,6,6,0.99)", border: isMobile ? "none" : "1px solid rgba(232,200,74,0.25)", borderTop: "1px solid rgba(232,200,74,0.25)", borderRadius: isMobile ? "20px 20px 0 0" : 16, overflow: "hidden", boxShadow: "0 -10px 60px rgba(0,0,0,0.8)" }}>
            <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(232,200,74,0.03)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button onClick={() => { setIsOpen(false); window.speechSynthesis.cancel() }}
                  style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", cursor: "pointer", fontSize: 22, lineHeight: 1 }}>×</button>
              </div>
            </div>

            <div style={{ padding: 16, minHeight: 80, maxHeight: isMobile ? 200 : 160, overflowY: "auto" }}>
              {error
            ...
            {inputMode === "voice" && (
              <div style={{ padding: "0 16px", paddingBottom: isMobile ? "calc(16px + env(safe-area-inset-bottom))" : "14px", display: "flex", gap: 8 }}>
                ...
                {response && <button onClick={() => { setDisplayTranscript(""); setResponse(""); setError(null); transcriptRef.current = "" }}
                  style={{ padding: "10px 14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 9, color: "rgba(255,255,255,0.3)", fontSize: 11, cursor: "pointer", fontFamily: "monospace" }}>CLR</button>}
              </div>
            )}

            {inputMode === "text" && (
              <form onSubmit={handleTextSubmit} style={{ padding: "0 16px", paddingBottom: isMobile ? "calc(16px + env(safe-area-inset-bottom))" : "14px", display: "flex", gap: 8 }}>
                <input value={textInput} onChange={e => setTextInput(e.target.value)} placeholder="Ask VEGA anything..." disabled={isThinking}
                ...
                <button type="submit" disabled={isThinking || !textInput.trim()}
                  style={{ padding: "9px 14px", background: "rgba(232,200,74,0.1)", border: "1px solid rgba(232,200,74,0.3)", borderRadius: 9, color: gold, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "monospace", opacity: isThinking || !textInput.trim() ? 0.4 : 1 }}>SEND</button>
              </form>
            )}
          </motion.div>
        )}
